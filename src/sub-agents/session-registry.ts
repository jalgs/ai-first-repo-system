...
export class SessionRegistryManager {
    ...
    private static registryPath(): string {
        return path.join(process.cwd(), 'sessions', SessionRegistryManager.current(), 'registry.json')
    }
    ...
    static list(): SessionRegistry[] {
        SessionRegistryManager.ensureFile()
        const raw = fs.readFileSync(SessionRegistryManager.registryPath(), { encoding: 'utf8' })
    ...
    static current(): string {
        if (!SessionRegistryManager.currentSessionId) {
            throw new Error('SessionRegistryManager: no hay sesión activa. Llama a setCurrent() primero.')
        }
